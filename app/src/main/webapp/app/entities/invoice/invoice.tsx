            ...
            <Table responsive>
              <thead>
                <tr>
                  ...
                  <th className="hand" onClick={sort('boltInvoice')}>
                    Bolt Invoice <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('sats')}>
                    Sats <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('settled')}>
                    Settled <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('paidByPubKey')}>
                    Paid By Pub Key <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('createdAt')}>
                    Created At <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('settledAt')}>
                    Settled At <FontAwesomeIcon icon="sort" />
                  </th>
                  <th>
              ...
              <tbody>
                {invoiceList.map((invoice, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{invoice.boltInvoice}</td>
                    <td>{invoice.sats}</td>
                    <td>{invoice.settled ? 'true' : 'false'}</td>
                    <td>{invoice.paidByPubKey}</td>
                    <td>
                      {invoice.createdAt ? <TextFormat type="date" value={invoice.createdAt} format={APP_LOCAL_DATE_FORMAT} /> : null}
                    </td>
                    <td>
                      {invoice.settledAt ? <TextFormat type="date" value={invoice.settledAt} format={APP_LOCAL_DATE_FORMAT} /> : null}
                    </td>
